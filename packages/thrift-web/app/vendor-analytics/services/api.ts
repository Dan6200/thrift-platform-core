...
const apiService = {
  async getKPIs(storeId: string, startDate: string, endDate: string) {
    const response = await axios.get(
      `${API_BASE_URL}/v1/dashboard/${storeId}/kpis`,
      {
  ...
  async getRevenueData(storeId: string, startDate: string, endDate: string) {
    const response = await axios.get(
      `${API_BASE_URL}/v1/dashboard/${storeId}/revenue-trends`,
      {
        params: { startDate, endDate },
      },
    )
  ...
  async getSalesData(storeId: string) {
    const response = await axios.get(
      `${API_BASE_URL}/v1/dashboard/${storeId}/sales-analytics`,
    )
    return response.data
  },
  ...
  async getCustomerData(storeId: string) {
    const response = await axios.get(
      `${API_BASE_URL}/v1/dashboard/${storeId}/customer-acquisition-trends`,
    )
    return response.data
  },
  async getProductPerformance(storeId: string) {
    const response = await axios.get(
      `${API_BASE_URL}/v1/dashboard/${storeId}/product-performance`,
    )
    return response.data
  },
